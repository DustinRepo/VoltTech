...
public class CombustionGeneratorMenu extends AbstractContainerMenu {
    ...
    public CombustionGeneratorMenu(int containerId, Inventory playerInventory, Container container, ContainerData containerData) {
        super(MenuRegistry.COMBUSTION_GENERATOR_MENU.get(), containerId);
        ...
        this.addSlot(new Slot(container, 0, 40, 54){
            @Override
            public boolean mayPlace(ItemStack stack) {
                return playerInventory.player.level().fuelValues().isFuel(stack);
            }
        });
        this.addSlot(new Slot(container,1, 120, 54) {
            @Override
            public boolean mayPlace(ItemStack stack) {
