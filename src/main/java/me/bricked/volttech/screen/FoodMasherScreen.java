...
public class FoodMasherScreen extends AbstractContainerScreen<FoodMasherMenu> {
    public FoodMasherScreen(FoodMasherMenu menu, Inventory playerInventory, Component title) {
        super(menu, playerInventory, title);
        this.constraints = Constraints.get(BlockRegistry.FOOD_MASHER.get());
    }
    private final Constraints constraints;
    private final ResourceLocation TEXTURE = VoltTech.resourceLocation("textures/gui/food_masher_gui.png");
    ...
    protected void renderBg(GuiGraphics guiGraphics, float v, int i, int i1) {
        ...
        int x = 76;
        int y = 37;
        float progress = Mth.clamp((float) menu.getMashingTicks() / (float) menu.getMaxMashingTicks(), 0, 1);
        int width = (int)(23 * progress);
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, this.leftPos + x, this.topPos + y, 0.0F, this.imageHeight, width, 12, width, 12, 256, 256);
    }
