...
public class FluidPipeRenderer implements BlockEntityRenderer<FluidPipeBlockEntity> {
    ...
    @Override
    public void render(FluidPipeBlockEntity fluidPipeBlockEntity, float v, PoseStack poseStack, MultiBufferSource multiBufferSource, int light, int overlay, Vec3 vec3) {
        if (fluidPipeBlockEntity.hasCover()) {
            ResourceLocation coverLocation = ResourceLocation.parse(fluidPipeBlockEntity.getCoverBlockID());
