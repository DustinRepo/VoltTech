...
public class ItemPipeRenderer implements BlockEntityRenderer<ItemPipeBlockEntity> {
    public ItemPipeRenderer(BlockEntityRendererProvider.Context context) {}
    @Override
    public void render(ItemPipeBlockEntity itemPipeBlockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource multiBufferSource, int light, int overlay, Vec3 vec3) {
        if (itemPipeBlockEntity.hasCover()) {
            ResourceLocation coverLocation = ResourceLocation.parse(itemPipeBlockEntity.getCoverBlockID());
