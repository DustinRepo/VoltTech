...
public class CrusherRecipeCategory implements IRecipeCategory<CrusherRecipe> {
    private static final ResourceLocation TYPE_RL = VoltTech.resourceLocation("crusher");
    public static final IRecipeType<CrusherRecipe> TYPE = new IRecipeType<>() {
        @Override
    ...
    public CrusherRecipeCategory(IGuiHelper helper) {
        ResourceLocation TEXTURE = VoltTech.resourceLocation("textures/gui/crusher_gui.png");
        this.background = helper.createDrawable(TEXTURE, 0, 0, 176, 80);
        this.background_bottom = helper.createDrawable(TEXTURE, 0, 160, 176, 6);
        this.icon = helper.createDrawableItemLike(ItemRegistry.CRUSHER_BLOCK_ITEM.get());
    }
    ...
    public Component getTitle() {
        return Component.translatable("block.volttech.crusher");
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, CrusherRecipe recipe, IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.OUTPUT, 107, 17).add(recipe.result());
        builder.addSlot(RecipeIngredientRole.OUTPUT, 107, 53).add(recipe.chanceResult());
    }
    ...
    public void draw(CrusherRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        ...
        guiGraphics.drawCenteredString(Minecraft.getInstance().font, StringifyUtil.translate("gui.chance_percent").append(Mth.floor(recipe.chance() * 100) + "%"), 116, 72, -1);
    }
}